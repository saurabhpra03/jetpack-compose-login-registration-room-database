...
fun RegistrationScreen(
    ...
    val updateName = { data: String ->
        name = data
        errorName = Validator.validateName(context, name.trim())
        isErrorName = errorName.isNotEmpty()
    }
    ...
    val updateEmailId = { data: String ->
        emailId = data
        errorEmailId = Validator.validateEmailId(context, emailId.trim())
    ...
    fun validation(): Boolean {
        ...
        return !(isErrorName || isErrorEmailId || isErrorPassword)
    }
    ...
    Column(
        ...
        AppBtn(btnName = stringResource(id = R.string.sign_up)) {
            if (validation()) {
                viewModel.checkEmailIdExists(Authentication(0, name.trim(), emailId.trim(), password.trim()))
            }
        }
        ...
        Text(
            modifier = Modifier
                .padding(top = 11.dp)
                .clickable {
                    navController.finishAndGoToNextScreen("sign_in")
                },
            text = stringResource(id = R.string.already_account_sign_in),
            style = TextStyle(
                fontSize = 15.sp,
